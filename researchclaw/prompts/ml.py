...
_DEFAULT_STAGES: dict[str, dict[str, Any]] = {
    ...
    "code_generation": {
        ...
        "user": (
            ...
            "risk.\n"
            "- RL/GRPO GEN SPEED & MEMORY: generation is the dominant GRPO cost "
            "and MUST use the KV cache. gradient_checkpointing forces "
            "use_cache=False → O(n²) decode (~7x slower, ~950s vs ~130s/step). For "
            "the GRPO trainer set gradient_checkpointing=False AND "
            "`model.config.use_cache=True`; rely on multi-GPU sharding for memory "
