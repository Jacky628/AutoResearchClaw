...
_DEFAULT_STAGES: dict[str, dict[str, Any]] = {
    ...
    "code_generation": {
        ...
        "user": (
            ...
            "oracle is a bug.\n"
            "- HARDWARE PLACEMENT: if the model fits on ONE GPU (compare the "
            "plan's min_vram_gb to one device's memory — a <=3B model in "
            "4-bit/bf16 easily fits a 24GB card), pin it to a single device "
            "(device_map={'': 0} or explicit .to('cuda:0')). Do NOT use "
            "device_map='auto' to shard a small model across GPUs: sharding "
            "forces a cross-GPU transfer every decode step, making generation "
            "several times slower for zero benefit. Shard ONLY when the model "
            "genuinely does not fit on one device.\n"
