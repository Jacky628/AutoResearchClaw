...
_DEFAULT_STAGES: dict[str, dict[str, Any]] = {
    ...
    "experiment_design": {
        ...
        "user": (
            ...
            "Add a top-level `environment` key declaring EVERYTHING the experiment "
            "code needs to run. Be honest and complete — this is checked against the "
            "real machine before code is written, and anything missing is flagged.\n"
            "Schema:\n"
            "  environment:\n"
            "    pip: [list of pip packages WITH versions, e.g. 'transformers>=4.40', 'peft', 'trl']\n"
            "    system: [OS/conda-level libs that pip cannot install, e.g. 'FreeCAD']\n"
            "    datasets:\n"
            "      - {name: <dataset>, source: <url-or-hf-id>, size_gb: <num>, cache: <path>}\n"
            "    compute: {gpu: required|optional, min_vram_gb: <num>, gpus: <num>}\n"
            "    est_setup_sec: <seconds to install+download before the run>\n"
            "Rules:\n"
            "- List ONLY what the plan actually uses. Prefer pip-installable packages; "
            "only put truly OS-level deps under `system`.\n"
            "- If a needed library/dataset is unlikely to be available or installable on "
            "the hardware above, PREFER a method that IS feasible here rather than "
            "assuming it exists — do not design around resources you cannot obtain.\n\n"
