...
_DEFAULT_STAGES: dict[str, dict[str, Any]] = {
    ...
    "code_generation": {
        ...
        "user": (
            ...
            "- SFT DATA MUST END WITH EOS (root cause of 'model never stops'): "
            "append tokenizer.eos_token to EVERY SFT training text "
            "(text = code + tokenizer.eos_token). Without it the model never "
            "learns to terminate → eos_frac=0 at eval, generations run to the cap, "
            "truncated/incomplete output, validity ~0. Only surfaces after real "
            "training (a 1-epoch smoke is under-trained so eos_frac=0 anyway and "
            "CANNOT catch it) — it silently invalidated a whole run.\n"
