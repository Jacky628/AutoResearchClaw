...
_DEFAULT_STAGES: dict[str, dict[str, Any]] = {
    ...
    "code_generation": {
        ...
        "user": (
            ...
            "`.isValid()`, not the OCC C++ spelling `IsValid()`).\n"
            ...
            "- HARDWARE PLACEMENT: choose placement at RUNTIME with an explicit "
            "conditional — do NOT hardcode single-GPU or 'auto'. Estimate the "
            "model's memory need (param count × bytes/param: 4-bit≈0.5, "
            "bf16/fp16≈2, fp32≈4; +~20% for optimizer/activations/RL reference "
            "copy) and compare to ONE device's free memory (torch.cuda."
            "mem_get_info / get_device_properties). If it fits on one device → "
            "pin to a single GPU (device_map={'': 0}) — common for <=3B models "
            "in 4-bit/bf16 on a 24GB card; only if it does NOT fit → shard with "
            "device_map='auto'. Print the decision, e.g. `PLACEMENT: single_gpu "
            "est_gb=3.1 free_gb=23.6`. Sharding a model that fits one card adds "
            "a cross-GPU transfer per decode step (several× slower for zero "
            "benefit); hardcoding single-GPU would OOM a model that needs "
            "sharding — the conditional handles both. CRITICAL when pinning "
            "single-GPU: ALSO set os.environ['CUDA_VISIBLE_DEVICES']='0' at the "
            "VERY TOP of the entry script (before importing torch/transformers), "
            "or a HuggingFace/TRL Trainer still sees n_gpu>1 and auto-wraps the "
            "model in torch.nn.DataParallel, whose scatter crashes with 'chunk "
            "expects at least a 1-dimensional tensor'. device_map={'': 0} pins "
            "the model but does NOT stop the Trainer's DataParallel — only "
            "limiting visible devices does.\n"
            ...
            "eval/proxy/collection samples→2, RL steps→1, dataset→a few rows — so a "
            "pre-flight runs ALL conditions end-to-end in 1-2 min and catches "
            "runtime crashes before the multi-hour run. Still touch every condition "
            "and the real tools/oracle; only shrink sizes.\n"
