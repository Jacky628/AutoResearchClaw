...
_DEFAULT_STAGES: dict[str, dict[str, Any]] = {
    ...
    "experiment_design": {
        ...
        "user": (
            ...
            "{sec_per_llm_generation: <s>, sec_per_oracle_call: <s>, "
            "sec_per_train_step: <s>}. State your assumptions — the runtime ETA "
            "calibration will check them.\n"
            "- WORST-CASE generation cost (CRITICAL): estimate sec_per_llm_generation "
            "for a generation that runs to the FULL max_new_tokens cap with NO early "
            "EOS — NOT the median/average length. Under-trained models routinely emit "
            "no EOS (eos_frac≈0) and ramble to the cap on every sample, so the cap "
            "length is the realistic per-generation cost the budget MUST survive. "
            "(Rule of thumb on a single 24GB GPU: ~30-40ms per generated token, so a "
            "512-token cap ≈ 15-20s, often more with oracle execution added.)\n"
            "- seconds_per_seed = sum(quantity × unit_cost) over that condition's "
            "phases (train steps, collection samples, eval samples, RL rollouts). "
            "Show the arithmetic, using the WORST-CASE generation cost above.\n"
            "- The budget MUST hold in the worst case: total over all conditions/seeds "
            "(with worst-case generation cost) MUST be < 0.8 × time_budget. If it does "
            "not fit, reduce in THIS order: (1) eval/collection/rollout SAMPLE COUNTS "
            "and RL step counts (these multiply the worst-case generation cost — they "
            "are the dominant term); (2) number of conditions; (3) LAST, seeds (2 is "
            "acceptable for a validation run; state the limitation). NEVER shrink unit "
            "costs or training epochs to make the numbers fit.\n"
            "- ANY phase with more than ~100 LLM generations or oracle/simulator "
            "executions must show its quantity × worst-case-unit-cost product fits the "
            "condition budget — otherwise REDUCE the quantity.\n\n"
